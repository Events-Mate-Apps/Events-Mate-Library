...
const QSectionDisplay: FC<{ section: Section }> = ({ section }) => {
  ...
  return (
    <Box p='10px' mb='10px'>
      {(section.titleContent && section.titleContent.translations) && <Text color={textColor} fontSize='28px' fontWeight='700' mb='20px'>
        {getCurrentTranslation(section.titleContent)}
      </Text>}
      {(section.descriptionContent && section.descriptionContent.translations) && <Text color={secondaryTextColor} fontSize='28px' fontWeight='500' mb='20px'>
